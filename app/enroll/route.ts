...
export async function GET(request: NextRequest) {
  ...
  if (!browserId || !version) {
    return NextResponse.json({ error: "browserId and version are required" }, { status: 400 });
  }
  ...
  const backend = await fetch(
    `${process.env.API_URL}/m/install?${new URLSearchParams({ uuid: browserId, extension: code }).toString()}`,
    {
      method: "POST",
